...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		.logout(logout -> logout
				...
				.deleteCookies("JSESSIONID") // Borra la cookie de sesión para asegurarte de que se cierre correctamente
				.permitAll()
				)
		.sessionManagement(session -> session
			    .invalidSessionUrl("/iniciarsesion?sessionExpired=true") // sesión inválida
			    .maximumSessions(1)
			    .maxSessionsPreventsLogin(false)
			    .expiredUrl("/iniciarsesion?sessionExpired=true") //sesión expirada
				);
